...
export async function apiGetCoffee(id: string) {
...
export async function apiUpdateMyProfile(profile: Profile) {
  return apiFetch<Profile>("/profiles/me", {
    method: "PUT",
...
export async function apiToggleFavorite(
  ...
  return apiFetch<{ isFavorite: boolean }>(`/favorites/${coffeeId}/toggle`, {
    method: "POST",
  });
}
...
export async function apiGetWeatherRecs(
  ...
  return apiFetch<RecsResponse>(`/recs/weather-live?${params.toString()}`);
}
